...
\newpage
\noindent We present now a strong and weak scaling study, Figures~\ref{blocked_device_ss} and~\ref{blocked_device_ws}, demonstrating the results of this approach:
\begin{table}[h]
	\begin{center}
		\begin{tabular}{|c| c|}
			\hline
...
\noindent The only other explanation we see as reasonable is that our \texttt{matmul-} style blocking incorporated in the Phi code likely does not wield the amount of threads available to us.  That is, were there more time, we would experiment with doing smaller sub-blocks to see how the performance changes.
