...
/**
 *  Copying the entire chunk of data, that a given processor has, into a 
 *  buffer array using local (n*ncols) indexing. This is done so as to
 *  remove data dependencies from the k-loop and making it the outermost
 *  loop. Moreover, we expect that copying the entire chunk of data at once,
 *  instead of one row every pass (of the outer loop), would help reduce the
 *  communication overhead.
 */

inline __attribute__((always_inline))
void chunk_copy(int *chunk, int *l, int ncols, int n)
{
	for (int m = 0; m < n*ncols; ++m)	{
		chunk[m] = l[m];
	}
}
...
int square(int nproc, int rank, int n, int nlocal,
           int* restrict lproc, int* restrict chunk_k)
{
	int ncols;
  int done      = 1;
  int col_shift = n / nproc;
	...
	for (int root = 0; root < nproc; ++root)	{
		...
		// Broadcast the value of number of columns owned by that processor
		MPI_Bcast(&ncols, 1, MPI_INT, root, MPI_COMM_WORLD);
		int* restrict chunk_k = (int*) calloc(n*ncols, sizeof(int));

		if (rank == root)	
			chunk_copy(chunk_k, lproc, ncols, n);

		MPI_Bcast(chunk_k, n * ncols, MPI_INT, root, MPI_COMM_WORLD);
 
		// Moving across columns of the local copied  matrix
