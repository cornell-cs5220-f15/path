...
int basic_floyd(const int lda, const int M, const int N, const int K,
                 const int * restrict l, const int * restrict lcopy, int * restrict lnew)
{
    ...
    int l_buf[ BLOCK_SIZE * BLOCK_SIZE ] __attribute__((aligned( 32 )));
    int lcopy_buf[ BLOCK_SIZE * BLOCK_SIZE ] __attribute__((aligned( 32 )));
    ...
    for( k = 0; k < K; ++k ) {
        for( i = 0; i < M; ++i ) {
            l_buf[ K * i + k ] = l[ i + lda * k ];
        }
    }
    
    for( j = 0; j < N; ++j ) {
        for( k = 0; k < K; ++k ) {
            lcopy_buf[ k + K * j ] = lcopy[ k + j * lda ];
        }
    }
    ...
    for ( i = 0; i < M; ++i ) {
        for ( j = 0; j < N; ++j ) {
            ...
            for ( k = 0; k < K; ++k ) {
                int lij = l_buf[ K * i + k ] + lcopy_buf[ k + K * j ];
              ...
              const int i, const int j, const int k)
{
    ...
    return basic_floyd( lda, M, N, K,
                l + i + lda * k, lcopy + k + lda * j, lnew + i + lda * j );
}
