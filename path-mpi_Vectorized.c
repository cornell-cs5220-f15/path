           ...
           int* restrict l)
{
    ...
    __m256i a00;
    __m256i a1,a2,a3,a4;
    int* tmp = _mm_malloc(n * sizeof(int), 32);
	...
	for (j =0; j<n; ++j)
      ...
      for (j = 0; j < size1; ++j) {
	int lkj1 = l[j*n+k];
	a00 = _mm256_set_epi32(lkj1,lkj1,lkj1,lkj1,lkj1,lkj1,lkj1,lkj1);
	c1 = l + j*n;
	for (i = 0; i < n; i+=8) {
	  a1 = _mm256_load_si256((__m256i *)(tmp+i));
	  a2 = _mm256_load_si256((__m256i *)(c1+i));
	  a3 = _mm256_add_epi32(a00,a1);
	  a4 = _mm256_min_epi32(a2,a3);
	  _mm256_store_si256((__m256i *)(c1+i),a4);
	  int test1 = _mm256_testz_si256(a2,a4);
	  if (test1 != 0) {
	    done = 0;
	  }
	}
...
{
    int size1=((rank+1)*n/np)-((rank)*n/np);
    int* local = _mm_malloc(n * size1 * sizeof(int), 32);    
